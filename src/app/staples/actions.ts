...
export async function createStapleRule(formData: FormData) {
  ...
  await prisma.stapleRule.create({
    data: {
      ...
      itemId,
      ruleType,
      defaultQuantity: num(formData.get("defaultQuantity")),
      ...
      expectedIntervalDays:
        ruleType === "restock" ? num(formData.get("expectedIntervalDays")) : null,
      lastPurchasedDate:
...
export async function updateStapleRule(formData: FormData) {
  ...
  if (formData.has("expectedIntervalDays")) {
    data.expectedIntervalDays = num(formData.get("expectedIntervalDays"));
